...
- Run the full line on all three asset briefs and all 10 stack drafts in one pass — same verdicts, same voice, every time
- Add French. No guide exists for it — derive the register rules from how the guide adapts DE and ES, state your assumptions, and produce one asset
- Handle a piece that's on-voice in EN but needs the DE market's harder-proof standard

**Demo target:** asset brief in → on-voice German asset out, every decision explained — then a language with no guide.
